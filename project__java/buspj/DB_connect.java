...
public class DB_connect {
    ...
    public Ticket ticket_load(String start, String end, String date) {
        ...
        ResultSet rs = null;
        Ticket t = new Ticket();
        String SQL = "SELECT starttime, company, db_class, seats, price FROM bus_table WHERE start = '" + start + "' and end = '" + end + "' and date ='" + date + "'";
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            conn =
                    DriverManager.getConnection("jdbc:mysql:" +
...
//            pstmt.setString(0, end);
            ...
            while (rs.next()) {
                ...
                String db_class = rs.getString(3);
                int seats = rs.getInt(4);
                int price = rs.getInt(5);

                t.insertTicket(starttime, company, db_class, seats, price);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void minus_seats(String start, String end, String date, String[] info) {
        ...
        try {
            ...
            String db_class = info[2];
            int price = Integer.valueOf(info[4]);

            String sql="update bus_table set seats=seats-1 where start=? and end=? and starttime=? and company=? and db_class=? and price=? and date=?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, start);
            pstmt.setString(2, end);
            pstmt.setString(3, starttime);
            pstmt.setString(4, company);
            pstmt.setString(5, db_class);
    ...
    public void plus_seats(String[] info) {
        ...
        try {
            ...
            String sql="update bus_table set seats=seats+1 where start='" + start +
                    ...
                    "' and db_class='" + db_class + "' and price=" + price + " and date='" + date + "'";
            pstmt = conn.prepareStatement(sql);
//            pstmt.setString(1, start);
//            pstmt.setString(2, end);
//            pstmt.setString(3, starttime);
...
//            pstmt.setString(7, date);
    ...
    public void saveUserReservation(String id, String start, String end, String date, int price, String[] info, int seatNum) {
        ...
        try {
            ...
            String sql="insert into reservation_user(id,start,end,date,starttime,company,db_class,price,seatnum)";
            sql += "values (?,?,?,?,?,?,?,?,?)";
            pstmt = conn.prepareStatement(sql);
    ...
    public CheckUp loadUserReservation(String id) {
        ...
        String SQL = "SELECT date, start, end, starttime,company, db_class, price, seatnum FROM reservation_user where id='"+ id +"'";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String db_class = rs.getString(6);
                String price = rs.getString(7);
                String seat = String.valueOf(rs.getInt(8));
                c.insertData(date,start,end,starttime, company, db_class, price, seat);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String[] load_information(String id) {
        ...
        try {
            ...
            String sql = "select start,end,date,starttime,company,db_class,price from reservation_user where id = '" + id + "'";
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery(sql);
